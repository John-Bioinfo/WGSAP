...
strdeal <- function(x) {
  ss <- tail(unlist(strsplit(x, split = "/")), n=1)
  unlist(strsplit(ss, "[.]"))[[1]]
}
sampname <- as.matrix(unlist(lapply(bamdir, strdeal)))
...
# Load DataBase
dbdir <- dirname(codexdir) 
# dbv <- head(system("ls -t /HOME/cheerland_1/WORKSPACE/pipeline/exome/db/CODEX/incremental/db", intern = T), n=1)
dbv <- basename(codexdir)
load(file.path(codexdir, paste(dbv, chr, 'db.rda', sep = '_')))
Y <- cbind(Y_db, Y)
sampname <- rbind(sampname_db, sampname)
...
prefix <- paste(dbdir, gt_time, sep = '/')
dir.create(prefix, showWarnings = F)
save(Y_db, sampname_db, gc, mapp, file = file.path(prefix, file = paste(gt_time, '_', chr, '_db', '.rda', sep='')), compress='xz')
