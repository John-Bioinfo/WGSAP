...
#my $Uniquely_mapped_to_genome=1;#spring change 0 to 1 at 2016.08.25
...
`$bin/../tool/mergeBed -i $outdir/freg_tmp.txt >$outdir/tmp.txt`;
`$bin/../tool/subtractBed -a $outdir/tmp.txt -b $regionfile >$outdir/freg.txt`;
#`rm $outdir/freg_tmp.txt $outdir/tmp.txt`; #spring add

$Initial_bases_near_target=`awk '{total+=\$3-\$2};END{print total}' $outdir/freg.txt`;
chomp($Initial_bases_near_target);

`$bin/samtools_v0.1.18/samtools depth -b $regionfile $bamfile >$outdir/target.depth`;
`$bin/samtools_v0.1.18/samtools depth -b $outdir/freg.txt $bamfile >$outdir/flanking.depth`;
...
=pod
	unless($fflag =~ /d/)
	{
		...
		if($_ =~ /X0:i:(\d+)/)
		{
			#$Uniquely_mapped_to_genome++ if($1==1);
		}
		if($_=~/XM:i:(\d+)/)
		{
...
=pod
open TMP,"$bin/samtools_v0.1.18/samtools view -X -L $regionfile $bamfile | " or die $!;
while(<TMP>)
{
	chomp;
	my $fflag=(split /\t/,$_)[1];
	unless($fflag =~ /d/)
	{
		if($_ =~ /X0:i:(\d+)/)
		{
			#$Uniquely_mapped_to_target++ if($1==1);
		}
		if($_=~/XM:i:(\d+)/)
		{
...
print STAT "Initial bases on target\t$Initial_bases_on_target\n";
print STAT "Initial bases near target\t$Initial_bases_near_target\n";
print STAT "Initial bases on or near target\t$Initial_bases_on_or_near_target\n";
...
printf STAT "Effective sequences near target(Mb)\t%.2f\n",$Effective_sequences_near_target/1000000;
printf STAT "Effective sequences on or near target(Mb)\t%.2f\n",$Effective_sequences_on_or_near_target/1000000;
#print STAT "Number of reads uniquely mapped to target\t$Uniquely_mapped_to_target\n";
#print STAT "Number of reads uniquely mapped to genome\t$Uniquely_mapped_to_genome\n";
printf STAT "Fraction of effective bases on target\t%.1f%%\n",100*$Fraction_of_effective_bases_on_target;
#printf STAT "Fraction of uniquely mapped on target\t%.1f%%\n",100*$Fraction_of_uniq_mapping_on_target;
printf STAT "Fraction of effective bases on or near target\t%.1f%%\n",100*$Fraction_of_effective_bases_on_or_near_target;
...
#printf STAT "Mismatch rate in target region\t%.2f%%\n",100*$Mismatch_rate_in_target_region;
#printf STAT "Mismatch rate in all effective sequence\t%.2f%%\n",100*$Mismatch_rate_in_all_effective_sequence;
...
printf STAT "Coverage of flanking region\t%.1f%%\n",100*$Coverage_of_flanking_region;
...
printf STAT "Fraction of flanking region covered with at least 20x\t%.1f%%\n",100*$Fraction_of_flanking_region_covered_with_at_least_20x;
printf STAT "Fraction of flanking region covered with at least 10x\t%.1f%%\n",100*$Fraction_of_flanking_region_covered_with_at_least_10x;
printf STAT "Fraction of flanking region covered with at least 4x\t%.1f%%\n",100*$Fraction_of_flanking_region_covered_with_at_least_4x;
...
#`rm $outdir/freg.txt $outdir/whole_genome.depth $outdir/target.depth $outdir/flanking.depth`;#spring add
system("rm $outdir/*.depth $outdir/*.txt") == 0 or die $!;
...
Rline
        ...
        system("Rscript $figFile.R") == 0 or die $!;
}
